        ...
        for conn in conns:
            ...
            try:
                ...
                if (
                    ...
                    and "SEARCH_INDEX_NOT_FOUND" not in str_err
                ):
                    # google memorystore does not support the DD argument, and
                    # redis flex does not support it either. in that case we drop
                    # the index without DD and flushall to clear the documents.
                    if (
                        "wrong number of arguments for FT.DROPINDEX command" in str_err
                        or "DD is not supported" in str_err
                    ):
                        print(
                            "Given the FT.DROPINDEX command with DD failed, we're "
                            "dropping the index without DD and flushing the entire DB..."
                        )
                        try:
