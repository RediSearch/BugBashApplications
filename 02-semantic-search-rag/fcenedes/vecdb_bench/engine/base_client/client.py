    ...
    for formatted_precision in precision_mapping.keys():
        ...
        for experiment_id, experiment_data in search_results.items():
            ...
            if format_precision_key(mean_precision) == formatted_precision and rps > best_rps:
                best_rps = rps
                # =================================================================
                # FIX: Handle different param structures across engines
                # Redis uses: params.search_params.ef
                # MongoDB uses: params.numCandidates (no search_params wrapper)
                # To revert: Remove the .get() fallback and use direct access
                # =================================================================
                search_params = experiment_data["params"].get("search_params", {})
                if not search_params:
                    # For engines like MongoDB that don't use search_params wrapper,
                    # extract all non-standard params as the search config
                    search_params = {
                        k: v for k, v in experiment_data["params"].items()
                        if k not in ("dataset", "experiment", "engine", "parallel")
                    }
                best_config = {
